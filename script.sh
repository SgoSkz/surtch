...
# NOTE: This was copied and pasted straight from ueberzug github and is used for creating an fzf selection screen
readonly BASH_BINARY="$(which bash)"
readonly REDRAW_COMMAND="toggle-preview+toggle-preview"
readonly REDRAW_KEY="µ"
declare -r -x DEFAULT_PREVIEW_POSITION="right"
declare -r -x UEBERZUG_FIFO="$(mktemp --dry-run --suffix "fzf-$$-ueberzug")"
declare -r -x IMAGE_TMP="$(mktemp --dry-run --suffix "preview-image")"
...
function draw_preview {
    curl -o ${IMAGE_TMP} $(./filter.sh ${@}) --silent
    calculate_position
